...
static bool parse_mail_methods(const char* script_json_path) {
    ...
    for (; iter != end && mail_hook_count < 32; ++iter) {
        ...
        // Filter out getters, setters, and backing fields
        ...
        // Check if it's a mail-related method (actual methods, not properties)
        ...
        if ((method_name.find("Mail") != std::string::npos ||
            ...
            method_name.length() > 3 && // Not too short
            method_name[0] >= 'A' && method_name[0] <= 'Z') { // Starts with capital (actual method)
            is_mail_method = true;
        }
        ...
        if (!is_mail_method) {
            ...
            // If we have offset, use: runtime_addr = il2cpp_base + offset
            // If we only have VA, calculate offset from dump VA: offset = va_dump - dump_base
            uint64_t runtime_va = 0;
            if (offset > 0 && il2cpp_base > 0) {
                runtime_va = il2cpp_base + offset;
            } else if (va_dump > 0 && il2cpp_base > 0) {
                // Try to calculate offset from dump VA (this is approximate)
                // We need the dump base address - for now, use a heuristic
                // If VA is in the typical range, assume it's relative to some base
                // Better approach: store dump base when dumping
                LOGW("Using VA without offset, may be inaccurate");
                runtime_va = va_dump; // Fallback to VA (may not work)
            } else {
                continue; // Can't calculate runtime address
            }
            ...
            LOGI("Found mail method: %s::%s::%s at 0x%" PRIx64 " (offset: 0x%" PRIx64 ")",
